...
namespace ConsoleApp
{
    internal class Program
    {
        static void Main(string[] args)
        {
            ...
            int playerDX = 0;
            int playerDY = 0;
            int allDots = 0; int collectDots = 0;
            ...
            while (isPlaying)
            {
                ...
                {
                    Move(ref playerX, ref playerY, playerDX, playerDY); CollectDots(map, playerX, playerY, ref collectDots);
                }
            ...
            if (collectDots == allDots)
            {
                Console.SetCursorPosition(0, 16); Console.WriteLine("Игра окончена. Вы победили!");
            }
        }
        ...
        static char[,] ReadMap(string mapName, out int playerX, out int playerY, ref int allDots, ref int collectDots)
        {
            playerX = 0;
            playerY = 0;
            string[] newFile = File.ReadAllLines($"Maps/{mapName}.txt"); char[,] map = new char[newFile.Length, newFile[0].Length];
            for (int i = 0; i < map.GetLength(0); i++)
            {
                for (int j = 0; j < map.GetLength(1); j++)
                {
